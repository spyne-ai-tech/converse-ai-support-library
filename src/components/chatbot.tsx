...
import type { Message, ChatbotProps } from '../types/components';
...
const Chatbot: React.FC<ChatbotProps> = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        {
            ...
            timestamp: new Date().toLocaleTimeString('en-US', CHAT_CONFIG.TIME_FORMAT_OPTIONS)
        }
    ]);
    ...
    const scrollToBottom = () => {
        messagesEndRef.current?.scrollIntoView({ behavior: CHAT_CONFIG.AUTO_SCROLL_BEHAVIOR });
    };
    ...
    useEffect(() => {
        const updateDateTime = () => {
            const now = new Date();
            const date = now.toLocaleDateString('en-US', CHAT_CONFIG.DATE_FORMAT_OPTIONS);
            setCurrentDate(date);
        };
    ...
    const sendChatMessage = async (message: string) => {
        ...
        try {
            ...
            const response = await VapiChatService.sendMessage(message, currentChatId);
